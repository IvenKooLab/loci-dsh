...
function apply(ctx, config) {
	const client = new LociClient(resolveConfig(config));
...
async function handleApiRequest(client, trustedHosts, req, res) {
	...
	try {
		writeJson(res, 200, {
			ok: true,
			...await dispatch(client, method, await readJsonBody(req))
		});
	} catch (error) {
...
async function dispatch(client, method, payload) {
	if (method === "ping") {
