/**
 * Browser-trust fence for the loci-dsh routes, behaviorally identical to the
 * /api gateway's fence in dsh-client-connection (src/api-request-trust.ts +
 * src/loopback-hostname.ts) and dsh-better-sidebar's copy of the same
 * (BSD-3-Clause): Host-header loopback or a configured trusted authority
...
export function isTrustedApiRequest(request: ApiTrustRequest, trustedHosts: readonly string[]): boolean {
  ...
  // hostname. Absent Origin is fine — the Host fence already bound the
  // request. The literal "null" (sandboxed iframes, file: pages) is refused.
